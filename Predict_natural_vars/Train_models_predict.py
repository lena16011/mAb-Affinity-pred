...
class GP_predict_native():
    ...
    def __init__(self, reg, model_name: str, metrics: dict):
        """
    ...
    def train_model(self, X_train, y_train, k=5, scoring=None):
        '''
        ...
        gp = GaussianProcessRegressor(kernel=kernel, random_state=123, n_restarts_optimizer=10)
...
# output
dir_out_model = os.path.join(ROOT_DIR, 'Predict_natural_vars/data/model_evaluation/log_transformed_input/Plots')
dir_out_eval = os.path.join(ROOT_DIR, 'Predict_natural_vars/data/model_evaluation/log_transformed_input')
...
data_test = pd.read_csv(input_test_seq).iloc[:8, :]
